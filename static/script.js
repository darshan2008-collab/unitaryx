...
(() => {
  ...
  if (canvas) {
    ...
    const COUNT = 40; // Reduced further to prevent video stuttering
  ...
  const reveals = document.querySelectorAll(
    '.reveal, .service-card, .project-card, .process-step, .why-card, .contact-wrapper'
  );
  ...
  const revealObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        entry.target.classList.add('animate'); // Supporting both naming conventions
  ...
  const openAdminModal = () => {
    if (adminModal) {
      ...
      setTimeout(() => (adminEmailInput || adminPassInput)?.focus(), 100);
    }
  };
    ...
    adminForm.addEventListener('submit', async e => {
      ...
      try {
        ...
        if (res.url.includes('/admin')) {
          ...
          if (adminError) adminError.textContent = 'Invalid admin credentials.';
          if (adminSubmit) {
